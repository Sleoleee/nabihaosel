...
def load_sales(db, year=None, branch=None):
    q = db.table("transactions").select(
        "new_row_total,document_number,customer_code,customer_name,slp_name,posting_date,branch,kategori"
    )
...
def leaderboard(year: Optional[str] = Query(None), branch: Optional[str] = Query(None)):
    ...
    for r in data:
        ...
        if r.get("document_number"): stats[sp]["docs"].add(r["document_number"])
        if r.get("customer_code"): stats[sp]["customers"].add(r["customer_code"])
    result = []
    for i, (sp, s) in enumerate(sorted(stats.items(), key=lambda x: -x[1]["revenue"])):
        bills, custs, rev = len(s["docs"]), len(s["customers"]), s["revenue"]
        result.append({"rank": i+1, "name": sp, "revenue": round(rev), "customers": custs, "bills": bills,
                        "aov": round(rev/bills) if bills else 0, "avg_per_customer": round(rev/custs) if custs else 0, "top_categories": []})
    return result
...
def drilldown(name: str, year: Optional[str] = Query(None)):
    db = get_client()
    q = db.table("transactions").select("new_row_total,kategori,customer_code,customer_name,posting_date").eq("slp_name", name)
    if year and year != "all": q = q.eq("year", int(year))
    data = q.order("posting_date").limit(ROW_LIMIT).execute().data
    monthly, cats = defaultdict(float), defaultdict(float)
    cust_stats = defaultdict(lambda: {"name": "", "revenue": 0, "last": ""})
    for r in data:
        pd_str = r.get("posting_date")
        if pd_str: monthly[int(pd_str[5:7])] += r.get("new_row_total") or 0
        ...
        if (pd_str or "") > cust_stats[code]["last"]: cust_stats[code]["last"] = pd_str or ""
    return {
        ...
        "customers": sorted([{"name": v["name"], "revenue": round(v["revenue"]), "last_purchase": v["last"]} for v in cust_stats.values()], key=lambda x: -x["revenue"]),
    }


@router.get("/scatter")
def scatter(year: Optional[str] = Query(None), branch: Optional[str] = Query(None)):
    return [{"name": r["name"], "customers": r["customers"], "revenue": r["revenue"]} for r in leaderboard(year, branch)]
