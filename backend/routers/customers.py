...
def load(db, year=None):
    q = db.table("transactions").select(
        "customer_code,customer_name,new_row_total,document_number,posting_date,status_payment"
    )
...
def build_stats(data):
    ...
    for r in data:
        ...
        if r.get("document_number"): stats[code]["docs"].add(r["document_number"])
...
def rfm(year: Optional[str] = Query(None)):
    ...
    if not all_dates: return {"matrix": [], "segments": []}
    cutoff = max(all_dates)
    customers = [{"code": c, "name": s["name"],
                  "recency": (date.fromisoformat(cutoff)-date.fromisoformat(max(s["dates"]))).days if s["dates"] else 9999,
                  "frequency": len(s["docs"]), "monetary": s["revenue"]} for c, s in stats.items()]
    if not customers: return {"matrix": [], "segments": []}
    ...
    def q4(val, vals, rev=False):
        n = len(vals); q25,q50,q75 = vals[n//4],vals[n//2],vals[3*n//4]
    ...
    for c in customers:
        matrix[(c["f_score"],c["m_score"])]["count"]+=1; matrix[(c["f_score"],c["m_score"])]["revenue"]+=c["monetary"]
        seg[c["segment"]]["count"]+=1; seg[c["segment"]]["revenue"]+=c["monetary"]
...
def tiers(year: Optional[str] = Query(None)):
    db = get_client()
    stats = build_stats(load(db))
    tier_data = defaultdict(lambda: {"count":0,"revenue":0,"customers":[]})
    for code, s in stats.items():
        avg = s["revenue"]/(len(s["months"]) or 1)
        t = get_customer_tier(avg)
        tier_data[t]["count"]+=1; tier_data[t]["revenue"]+=s["revenue"]; tier_data[t]["customers"].append(s["name"])
    tier_order=["Tier 1 — ≥30jt","Tier 2 — 20–30jt","Tier 3 — 15–20jt","Tier 4 — 10–15jt","Tier 5 — 7–10jt",
                "Tier 6 — 6–7jt","Tier 7 — 5–6jt","Tier 8 — 4–5jt","Tier 9 — 3–4jt","Tier 10 — 2–3jt",
                "Tier 11 — 1–2jt","Tier 12 — 500rb–1jt","Tier 13 — <500rb"]
...
def customer_list(year: Optional[str] = Query(None), tier: Optional[str] = Query(None),
    ...
    for code, s in stats.items():
        ...
        rows.append({"code":code,"name":s["name"],"tier":get_customer_tier(avg),"revenue":round(s["revenue"]),
                     "avg_monthly":round(avg),"bills":len(s["docs"]),"active_months":len(s["months"]),
