...
def kategori_trend(year: Optional[str] = Query(None), kategori: Optional[str] = Query(None)):
    db = get_client()
    data = db.table("transactions").select("new_row_total,kategori,year").limit(ROW_LIMIT).execute().data
...
def kategori_growth(year: Optional[str] = Query(None)):
    ...
    for k, yrs in grouped.items():
        curr = yrs.get(compare_year, 0)
        prev = yrs.get(prev_year, 0) if prev_year else 0
...
def top_products(year: Optional[str] = Query(None), kategori: Optional[str] = Query(None)):
    ...
    result = sorted([{"item_no": k, "description": v["description"], "kategori": v["kategori"],
